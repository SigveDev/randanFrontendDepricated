...
const Chapter = ({ chapters, user }) => {
    ...
    useEffect(() => {
        const getChapter = async () => {
            ...
            if(!pageAccessedByReload) {
                setTimeout(() => {
                    axios.put("http://node.binders.net:8123/vidstats/update/views/" + id, {});
                }, 5000);
            }
        }
        getChapter();
    ...
    const like = async () => {
        const url = window.location.pathname.split("/");
        const id = url[url.length - 1];
        const res = await axios.post("http://node.binders.net:8123/liked/like/" + id, {
            userId: user._id
        ...
        if(res.status === 200) {
            ...
            setLiked(res.data);
            if(likeButton === false) {
                await axios.put('http://node.binders.net:8123/vidstats/update/likes/add/' + id, {});
            } else if(likeButton === true) {
                await axios.put('http://node.binders.net:8123/vidstats/update/likes/remove/' + id, {});
            }
        }
    }
                ...
                <div className="pages-content">
                    {chapter.pages.map((page, i) => (
                        <a href={`http://localhost:3000/page/${chapter._id}/${i + 1}?full=false`} key={i}>
                            <img src={"http://node.binders.net:8123/uploads/" + page.image} alt={"page " + i} className="pageImg" />
                            <p>Page {i + 1}</p>
                        </a>
                    ))}
                </div>
