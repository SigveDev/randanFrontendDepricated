    ...
    const handleSubmit = async (e) => {
        ...
        if(pages !== 0) {
            ...
            for(let i = 0; i < chapter.pages.length; i++) {
                axios.delete("http://localhost:5000/upload/delete/" + chapter.pages[i].image);
            }
        } else {
        ...
        if (imageUrl) {
            ...
            axios.delete("http://localhost:5000/upload/delete/" + chapter.image);
        } else {
            newChapter.image = chapter.image;
        }
        try {
            await axios.put("http://localhost:5000/chapter/update/" + chapter._id, newChapter);
        ...
        <div className="edit">
            ...
            {chapter && <form onSubmit={handleSubmit} className="edit-form">
                ...
                <div className="left">
                    ...
                    {imageUrl ? <img src={imageUrl} alt="Uploaded file" className="uploadedImg" onClick={handleClick} /> : <img src={"http://localhost:5000/uploads/" + chapter.image} alt="Please upload" className="uploadedImg" onClick={handleClick} />}
                </div>
                <div className="right">
                ...
                <div className="pages-grid">
                    {imagesUrl.length === 0 ? chapter.pages.map((page, i) => (
                        <div key={i}>
                            <img src={"http://localhost:5000/uploads/" + page.image} alt={"page " + i} className="uploadedPage" />
                            <p>Page {i + 1}</p>
                        </div>
                    )) : 
                ...
                <div className="submit">
                    <input type="button" value="Delete" onClick={() => {
                        if (window.confirm("Are you sure you want to delete this chapter?")) {
                            axios.delete("http://localhost:5000/chapter/delete/" + chapter._id, {data: {userId: user._id}, withCredentials: true});
                            window.location.replace("/admin?page=my");
                        }
                    }} />
