    ...
    useEffect(() => {
        const changeReading = async () => {
            ...
            if(temp === undefined) {
                ...
                const res = await axios.delete("http://node.binders.net:8123/liked/reading/" + user._id, {
                ...
                if(liked.liked.length !== 0) {
                    liked.liked.forEach(like => {
                        if(chapters.find(chapter => chapter._id === like) === undefined) {
                            const res = axios.put("http://node.binders.net:8123/liked/like/" + user._id, {
                                id: like
                            }, { withCredentials: true });
                            if(res.status === 200) {
                                localStorage.setItem('likes', JSON.stringify(res.data));
                ...
                if(liked.history.length !== 0) {
                    liked.history.forEach(history => {
                        if(chapters.find(chapter => chapter._id === history) === undefined) {
                            const res = axios.put("http://node.binders.net:8123/liked/history/" + user._id, {
                                id: history
                            }, { withCredentials: true });
                            if(res.status === 200) {
                                localStorage.setItem('likes', JSON.stringify(res.data));
